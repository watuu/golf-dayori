<div class="front_page_section front_page_section_testimonials<?php
	$putter_scheme = putter_get_theme_option( 'front_page_testimonials_scheme' );
	if ( ! empty( $putter_scheme ) && ! putter_is_inherit( $putter_scheme ) ) {
		echo ' scheme_' . esc_attr( $putter_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( putter_get_theme_option( 'front_page_testimonials_paddings' ) );
	if ( putter_get_theme_option( 'front_page_testimonials_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$putter_css      = '';
		$putter_bg_image = putter_get_theme_option( 'front_page_testimonials_bg_image' );
...
<?php
	// Add anchor
	$putter_anchor_icon = putter_get_theme_option( 'front_page_testimonials_anchor_icon' );
	$putter_anchor_text = putter_get_theme_option( 'front_page_testimonials_anchor_text' );
...
?>
	...
	<?php
	if ( putter_get_theme_option( 'front_page_testimonials_fullheight' ) ) {
	...
	?>
			...
			<?php
			$putter_css      = '';
			$putter_bg_mask  = putter_get_theme_option( 'front_page_testimonials_bg_mask' );
			$putter_bg_color_type = putter_get_theme_option( 'front_page_testimonials_bg_color_type' );
			if ( 'custom' == $putter_bg_color_type ) {
				$putter_bg_color = putter_get_theme_option( 'front_page_testimonials_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_testimonials_content_wrap content_wrap">
			<?php
			// Caption
			$putter_caption = putter_get_theme_option( 'front_page_testimonials_caption' );
			if ( ! empty( $putter_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_testimonials_caption front_page_block_<?php echo ! empty( $putter_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $putter_caption, 'putter_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$putter_description = putter_get_theme_option( 'front_page_testimonials_description' );
			if ( ! empty( $putter_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_testimonials_description front_page_block_<?php echo ! empty( $putter_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $putter_description ), 'putter_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_testimonials_output">
				...
				if ( is_active_sidebar( 'front_page_testimonials_widgets' ) ) {
					...
					if ( ! putter_exists_trx_addons() ) {
						...
						putter_customizer_need_widgets_message( 'front_page_testimonials_caption', 'ThemeREX Addons - Testimonials' );
					}
				}
				?>
			</div>
