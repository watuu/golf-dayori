...
if ( ! defined( 'PUTTER_THEME_FREE' ) ) {
	define( 'PUTTER_THEME_FREE', false );
}
if ( ! defined( 'PUTTER_THEME_FREE_WP' ) ) {
	define( 'PUTTER_THEME_FREE_WP', false );
}

// If this theme support external updates
if ( ! defined( 'PUTTER_THEME_ALLOW_UPDATE' ) ) {
	define( 'PUTTER_THEME_ALLOW_UPDATE', true );
}

// If this theme uses skins
if ( ! defined( 'PUTTER_ALLOW_SKINS' ) ) {
	define( 'PUTTER_ALLOW_SKINS', true );
}
if ( ! defined( 'PUTTER_DEFAULT_SKIN' ) ) {
	define( 'PUTTER_DEFAULT_SKIN', 'default' );
}
if ( ! defined( 'PUTTER_REMEMBER_SKIN' ) ) {
	define( 'PUTTER_REMEMBER_SKIN', false );
}
...
if ( ! function_exists( 'putter_get_theme_pro_key' ) ) {
	/**
	 ...
	 */
	function putter_get_theme_pro_key() {
		$theme_pro_key = get_option( sprintf( 'purchase_code_src_%s', get_template() ) );
		if ( empty( $theme_pro_key ) ) {
			$theme_pro_key = putter_storage_get( 'theme_pro_key' );
		}
		return $theme_pro_key;
	}
}

if ( ! function_exists( 'putter_trx_updater_get_theme_info' ) ) {
	add_filter( 'trx_updater_filter_get_theme_info', 'putter_trx_updater_get_theme_info' );
	/**
	 * Add an information about the theme engine to the array with a theme info.
	 *
	 * Hooks: add_filter( 'trx_updater_filter_get_theme_info', 'putter_trx_updater_get_theme_info' );
	 *
	 * @param array $info  An array with a theme information.
	 *
	 * @return array       A theme information with an engine info.
	 */
	function putter_trx_updater_get_theme_info( $info ) {
		$engine = putter_storage_get( 'engine' );
...
if ( ! function_exists( 'putter_original_theme_slug' ) ) {
	add_filter( 'putter_filter_original_theme_slug', 'putter_original_theme_slug' );
	/**
	 ...
	 *
	 * Hooks: add_filter( 'putter_filter_original_theme_slug', 'putter_original_theme_slug' );
	 *
	 * @param string $theme_slug  Current slug of the theme.
	 *
	 * @return string             Original slug of the theme.
	 */
	function putter_original_theme_slug( $theme_slug = '' ) {
		return 'putter';
	}
}
...
if ( ! function_exists( 'putter_importer_set_options' ) ) {
	add_filter( 'trx_addons_filter_importer_options', 'putter_importer_set_options', 9 );
	/**
	 * Set a theme-specific options for the import of the demo data via plugin ThemeREX Addons.
	 *
	 * Hooks: add_filter( 'trx_addons_filter_importer_options', 'putter_importer_set_options', 9 );
	 *
	 * @param array $options  Options for the import of the demo data.
	 *
	 * @return array          Processed options with a theme-specific values.
	 */
	function putter_importer_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			...
			// Prepare demo data
			$options['demo_url'] = esc_url( putter_get_protocol() . ':' . putter_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
			$options['required_plugins'] = array_keys( putter_storage_get( 'required_plugins' ) );
...
if ( ! function_exists( 'putter_ocdi_set_options' ) ) {
	add_filter( 'trx_addons_filter_ocdi_options', 'putter_ocdi_set_options', 9 );
	/**
	 ...
	 * @return array          Processed options with a theme-specific values.
	 */
	function putter_ocdi_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			// Prepare demo data
			$options['demo_url'] = esc_url( putter_get_protocol() . ':' . putter_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
...
if ( ! function_exists( 'putter_customizer_theme_setup1' ) ) {
	add_action( 'after_setup_theme', 'putter_customizer_theme_setup1', 1 );
	/**
	 * Create an internal setting for the theme.
	 * Attention! This settings ONLY FOR PROGRAMMERS, NOT FOR CUSTOMER
	 *
	 * Hooks: add_action( 'after_setup_theme', 'putter_customizer_theme_setup1', 1 );
	 *
	 ...
	 */
	function putter_customizer_theme_setup1() {
		putter_storage_set(
			'settings', array(
				...
				'allow_fullscreen'          => false,                   // Allow cases 'fullscreen' and 'fullwide' for the body style in the Theme Options
																		...
																		// (can be removed if filter 'putter_filter_allow_fullscreen' return false)
