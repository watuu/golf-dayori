...
if ( function_exists( 'putter_elementor_is_preview' ) && putter_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			putter_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	putter_new_main_query(
		array(
			'post_type'      => putter_get_theme_option( 'post_type' ),
			'category'       => putter_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'putter_filter_get_template_part', putter_blog_archive_get_template() ) );
}
